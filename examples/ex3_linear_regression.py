...
# 2. CHIA TẬP TRAIN/TEST BẰNG NUMPY (Thay thế train_test_split của sklearn)
# Thiết lập seed để đảm bảo kết quả cố định giống random_state=42
np.random.seed(42)

# Xáo trộn chỉ số (indices) ngẫu nhiên
shuffled_indices = np.random.permutation(len(X))
test_size = 0.2
test_set_size = int(len(X) * test_size)

# Tách chỉ số cho tập test và train
test_indices = shuffled_indices[:test_set_size]
train_indices = shuffled_indices[test_set_size:]

# Trích xuất dữ liệu dựa trên chỉ số đã chia
...
# 4. TỐI ƯU HÓA MAP (RIDGE REGRESSION)
...
# 5. TỐI ƯU HÓA MLE (ORDINARY LEAST SQUARES)
...
mle_weights = optimizer_mle.minimize(
    ...
    grad_func=grad_ols,
    data=data_tuple,
    lmbda=0,  # lambda = 0, không có regularization
    sigma=sigma_likeli
)
...
sigma_likeli = 0.5      # nhiễu của likelihood
sigma_prior = 0.4       # prior std (điều khiển độ co) - giảm xuống 0.5 để thấy rõ shrinkage
lambda_prior = 1.0 / (sigma_prior**2)
learning_rate = 1e-5
epochs = 5000
